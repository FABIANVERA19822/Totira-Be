...
namespace Totira.Business.ThirdPartyIntegrationService.Handlers.Commands.Persona
{
    public class TenantPersonaValidationCommandHandler : IMessageHandler<TenantPersonaValidationCommand>
    {
        ...
        public async Task HandleAsync(IContext context, Either<Exception, TenantPersonaValidationCommand> command)
        {
            await command.MatchAsync(async cmd => {
                var templateId = cmd.data.attributes.payload.included.FirstOrDefault(x => x.type == "inquiry-template").id;

                _logger.LogInformation("Front-TemplatePersona={front} Back-TemplatePersona={back}", templateId, _personaOptions.TemplateId);
                ...
                var tenantId = Guid.Parse(cmd.data.attributes.payload.data.attributes.referenceid);
                ...
                var currentInquiry = await _tenantPersonalValidationRepository.GetByIdAsync(cmd.data.attributes.payload.data.id);

            if (currentInquiry == null)
            {
                await CreateInquiryTenant(cmd, tenantId);
                ...
                {
                    if (currentInquiry.Status != cmd.data.attributes.payload.data.attributes.status)
                    {
                        await UpdateInquityTenant(currentInquiry, cmd);
