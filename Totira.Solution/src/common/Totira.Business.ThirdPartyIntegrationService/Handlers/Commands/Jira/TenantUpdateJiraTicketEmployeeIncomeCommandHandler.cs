...
namespace Totira.Business.ThirdPartyIntegrationService.Handlers.Commands.Jira
{
    public class TenantUpdateJiraTicketEmployeeIncomeCommandHandler : IMessageHandler<UpdateTenantEmployeeAndIncomeTicketJiraCommand>
    {
        ...
        public async Task HandleAsync(IContext context, Either<Exception, UpdateTenantEmployeeAndIncomeTicketJiraCommand> command)
        {
            await command.MatchAsync(async cmd => {
                _logger.LogInformation("updating information from jira for issue {IssueId}", cmd.Issue.id);
                ...
                var inquiryUpdatedEvent = new TenantJiraTicketEmployeeIncomeUpdatedEvent(Guid.NewGuid());
                throw ex;
            });
        }
    }
