...
namespace Totira.Business.ThirdPartyIntegrationService.Handlers.Commands.Jira
{
    public class TenantCreateJiraTicketEmployeeIncomeCommandHandler : IMessageHandler<TenantEmployeeAndIncomeTicketJiraCommand>
    {
        ...
        public async Task HandleAsync(IContext context, Either<Exception, TenantEmployeeAndIncomeTicketJiraCommand> command)
        {
            await command.MatchAsync(async cmd => {
                var urlIncomes = $"{_restClientOptions.User}/user/tenant/{cmd.TenantId}/incomes";
                ...
                _logger.LogInformation($"Starting procces for send information of employeement and income to jira tenant id {cmd.TenantId}");
                ...
                if (incomes.Content.StudyDetails is not null)
                    ...
                    {
                        var urlFiles = $"{_restClientOptions.User}/userFiles/{cmd.TenantId}/{study.StudyId}";
                ...
                {
                    var urlFiles = $"{_restClientOptions.User}/userFiles/{cmd.TenantId}/{inc.IncomeId}";
                    var filesResponse = await _queryRestClient.GetAsync<GetIncomeFilesDto>(urlFiles);
                ...
                Expression<Func<TenantVerifiedProfile, bool>> expression = p => p.TenantId == cmd.TenantId;
