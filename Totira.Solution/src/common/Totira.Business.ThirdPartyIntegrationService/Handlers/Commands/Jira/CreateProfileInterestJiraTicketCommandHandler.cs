...
namespace Totira.Business.ThirdPartyIntegrationService.Handlers.Commands.Jira
{
    public class CreateProfileInterestJiraTicketCommandHandler : IMessageHandler<CreateProfileInterestJiraTicketCommand>
    {
        ...
        public async Task HandleAsync(IContext context, CreateProfileInterestJiraTicketCommand command)
        {
            ...
            var shareProfileUrl = $"{_restClientOptions.User}/user/tenant/{command.TenantId}/{encryptedAccessCode}/{command.AgentEmail}/shareProfile";
            var shareProfile = await _queryRestClient.GetAsync<GetTenantShareProfileDto>(shareProfileUrl);

            var contactInfoUrl = $"{_restClientOptions.User}/user/tenant/contactinfo/{command.TenantId}";
            ...
            List<Contents> contents = BuildDescriptionTicket(basicInfo.Content, shareProfile.Content, contactInfo.Content, command.ProfileAccessCode.ToString(), command.AgentEmail, link);
