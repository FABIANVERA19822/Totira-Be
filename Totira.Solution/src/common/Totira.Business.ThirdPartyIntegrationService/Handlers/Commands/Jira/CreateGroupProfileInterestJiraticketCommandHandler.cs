...
namespace Totira.Business.ThirdPartyIntegrationService.Handlers.Commands.Jira
{
	public class CreateGroupProfileInterestJiraticketCommandHandler : IMessageHandler<CreateGroupProfileInterestJiraticketCommand>
    {
        ...
        public async Task HandleAsync(IContext context, Either<Exception, CreateGroupProfileInterestJiraticketCommand> command)
        {
            await command.MatchAsync(async cmd => {
                var GroupApplicationSummaryUrl = $"{_restClientOptions.User}/user/tenant/GroupApplicationSummary/{cmd.TenantId}";
            ...
            var shareProfileUrl = $"{_restClientOptions.User}/user/CheckTenantGroupShareProfileForCheckCode/{cmd.TenantId}/{cmd.ProfileAccessCode}/{cmd.AgentEmail}";
            ...
            if (GroupApplicationSummary?.Content?.CoApplicants != null)
            {
                ...
                List<Contents> contents = BuildDescriptionTicket(BasicInfoAndContactInfo, shareProfile.Content, cmd.ProfileAccessCode.ToString(), cmd.AgentEmail, link);
