...
public class ApplySoftCheckCommandHandler : IMessageHandler<ApplySoftCheckCommand>
{
    ...
    public async Task HandleAsync(IContext context, Either<Exception, ApplySoftCheckCommand> message)
    {
        await message.MatchAsync(async msg => {
            var tenant = await _tenantApplicationsRepository.GetByIdAsync(msg.TenantId.ToString()) ??
                TenantApplications.Empty(msg.TenantId.ToString());
            ...
            var response = await _certnHandler.PostAsync("applications/quick/", msg.Request);
