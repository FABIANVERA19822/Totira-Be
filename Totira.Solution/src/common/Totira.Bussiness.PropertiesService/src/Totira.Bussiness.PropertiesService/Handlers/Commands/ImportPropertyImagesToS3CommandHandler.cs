...
namespace Totira.Bussiness.PropertiesService.Handlers.Commands
{
    public class ImportPropertyImagesToS3CommandHandler : IMessageHandler<ImportPropertyImagesToS3Command>
    {
       ...
       public async Task HandleAsync(IContext context, Either<Exception, ImportPropertyImagesToS3Command> command)
        {
            await command.MatchAsync(async cmd => {
                await _client.Connect();

                List<PhotoIdAlpha> photoIds = new List<PhotoIdAlpha>() { new PhotoIdAlpha(cmd.PropertyId) };
                ...
                foreach (var image in propertyImages)
                {
                    ...
                    var keyName = GetFormattedKeyName(
                            ...
                            fileName: cmd.PropertyId + imageOrder.ToString());
                    ...
                    var propertyImageUploadedEvent = new PropertyImageUploadedEvent(cmd.PropertyId);
                }
                SavePropertyImagesToDB(propertyImages, cmd.PropertyId);
            }, ex => throw ex);
        }
        private static string GetFormattedKeyName(string propertyId, string fileName) => $"{propertyId}/{fileName}";
