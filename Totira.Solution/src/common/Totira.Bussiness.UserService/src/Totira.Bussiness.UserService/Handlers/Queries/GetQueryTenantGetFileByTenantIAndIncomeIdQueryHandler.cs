...
namespace Totira.Bussiness.UserService.Handlers.Queries
{
    public class GetQueryTenantGetFileByTenantIAndIncomeIdQueryHandler : IQueryHandler<QueryTenantGetFileByTenantIAndIncomeId, GetIncomeFilesDto>
    {
        ...
        public async Task<GetIncomeFilesDto> HandleAsync(QueryTenantGetFileByTenantIAndIncomeId query)
        {
            ...
            {
                var filesAux = result.EmployeeIncomes.FirstOrDefault(ei => ei.Id == query.IncomeId)?.Files ?? new List<Domain.Common.File>();
            ...
            {
                var filesAux = result.StudentIncomes.FirstOrDefault(ei => ei.Id == query.IncomeId)?.IncomeProofs ?? new List<Domain.Common.File>();
                foreach (var file in filesAux)
                {
                    var fileDto = new DTO.File();
                ...
                filesAux = result.StudentIncomes.FirstOrDefault(ei => ei.Id == query.IncomeId)?.EnrollmentProofs ?? new List<Domain.Common.File>();
                foreach (var file in filesAux)
                {
                    var fileDto = new DTO.File();
                ...
                filesAux = result.StudentIncomes.FirstOrDefault(ei => ei.Id == query.IncomeId)?.StudyPermitsOrVisas ?? new List<Domain.Common.File>();
                foreach (var file in filesAux)
                {
                    var fileDto = new DTO.File();
