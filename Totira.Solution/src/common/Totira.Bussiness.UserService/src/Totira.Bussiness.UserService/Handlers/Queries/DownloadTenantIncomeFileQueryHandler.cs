...
public class DownloadTenantIncomeFileQueryHandler : IQueryHandler<QueryDownloadTenantIncomeFile, DownloadFileDto>
{
    ...
    public async Task<DownloadFileDto> HandleAsync(QueryDownloadTenantIncomeFile query)
    {
        ...
        if (tenant.StudentIncomes is not null && tenant.StudentIncomes.Any())
        {
            ...
            var files = new List<Domain.Common.File>();
