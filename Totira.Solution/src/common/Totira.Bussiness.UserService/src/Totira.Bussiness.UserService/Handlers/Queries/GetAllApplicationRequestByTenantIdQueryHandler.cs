            ...
            if (applicationRequest.Any())
            {
                foreach (var application in applicationsRequest)
                {
                    var mapped = new GetTenantApplicationRequestDto()
                    {
                        ...
                        IsMulti = application.Coapplicants != null && application.Coapplicants.Count() > 0,
                        CreatedOn = application.CreatedOn,
                        Coapplicants = application.Coapplicants != null ? application.Coapplicants.Select(co => new CoApplicantDto { CoapplicantId = co.Id, Email = co.Email, Status = co.Status }).ToList() : null,
                        Guarantor = application.Guarantor != null ? new CoApplicantDto { CoapplicantId = application.Guarantor.Id, Email = application.Guarantor.Email, Status = application.Guarantor.Status } : null,
