    ...
    public class GetApplicationRequestByTenantIdQueryHandler : IQueryHandler<QueryApplicationRequestByTenantId, GetTenantApplicationRequestDto>
    {
        ...
        public async Task<GetTenantApplicationRequestDto> HandleAsync(QueryApplicationRequestByTenantId query)
        {
            ...
            var result = new GetTenantApplicationRequestDto()
            {
                ...
                IsMulti = applicationRequest.Coapplicants != null && applicationRequest.Coapplicants.Count() > 0,
                CreatedOn = applicationRequest.CreatedOn,
                Coapplicants = applicationRequest.Coapplicants != null ? applicationRequest.Coapplicants.Select(co => new CoApplicantDto { CoapplicantId = co.Id, Email = co.Email, Status = co.Status }).ToList() : null,
                Guarantor = applicationRequest.Guarantor != null ? new CoApplicantDto { CoapplicantId = applicationRequest.Guarantor.Id, Email = applicationRequest.Guarantor.Email, Status = applicationRequest.Guarantor.Status }: null,
