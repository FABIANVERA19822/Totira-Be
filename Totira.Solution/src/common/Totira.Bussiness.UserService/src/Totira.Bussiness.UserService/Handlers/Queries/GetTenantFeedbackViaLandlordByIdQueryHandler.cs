...
namespace Totira.Bussiness.UserService.Handlers.Queries
{
    public class GetTenantFeedbackViaLandlordByIdQueryHandler : IQueryHandler<QueryTenantFeedbackViaLandlordById, GetTenantFeedbackViaLandlordDto>
    {
        ...
        public async Task<GetTenantFeedbackViaLandlordDto> HandleAsync(QueryTenantFeedbackViaLandlordById query)
        {
            ...
            {
                TenantName = historyInfo.FullName
            };
            ...
            {
                response.Status = historyInfo.CreatedOn.AddDays(_configuration.Value.timeoutExpirationDaysRequest) < DateTimeOffset.Now ? "Expired" : "Pending";
            }
            else
            {
    ...
    public static class LandlordById
    {
        ...
        {
            var data = (await tenantRentalHistoryRepositorys.Get(r => r.RentalHistories.Any(rf => rf.Id == landlordId))).FirstOrDefault();
            return data is null ? null : data.RentalHistories.First(r => r.Id == landlordId);
        }
    }
}
