...
namespace Totira.Bussiness.UserService.Handlers.Commands
{
    public class UpdateGroupApplicationCommandHandler : IMessageHandler<UpdateGroupApplicationCommand>
    {
        ...
        public async Task HandleAsync(IContext context, UpdateGroupApplicationCommand command)
        {
            foreach (var applicationDto in command.GroupApplicationProfiles)
            {
                _logger.LogDebug($"updating tenant Application Group profile for tenant {applicationDto.TenantId}");
