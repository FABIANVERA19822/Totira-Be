...
namespace Totira.Bussiness.UserService.Handlers.Commands
{
	public class DeleteTenantUnacceptedApplicationRequestCommandHandler : IMessageHandler<DeleteTenantUnacceptedApplicationRequestCommand>
    {
        ...
        public async Task HandleAsync(IContext context, Either<Exception, DeleteTenantUnacceptedApplicationRequestCommand> command)
        {
            await command.MatchAsync(async cmd => {
                var basicInfo = (await _basicInfoRepository.Get(x => x.TenantEmail == cmd.Email)).FirstOrDefault();

                Expression<Func<TenantApplicationRequest, bool>> requestExpression = r =>
                                r.Id != cmd.RequestId &&
                                (
                                    r.Coapplicants.Any(ca => ca.Email == cmd.Email) ||
                                    r.Guarantor.Email == cmd.Email ||
                ...
                var userCreatedEvent = new TenantUnacceptedApplicationRequestDeletedCommandEvent(Guid.Empty);
                var notificationContext = _contextFactory.Create(string.Empty, context.CreatedBy);
                var messageOutboxId = await _messageService.SendAsync(notificationContext, userCreatedEvent);
                throw ex;
            });
