...
namespace Totira.Bussiness.UserService.Handlers.Commands
{
    public class UpdateTenantApplicationRequestCommandHandler : IMessageHandler<UpdateTenantApplicationRequestCommand>
    {
        ...
        /// <returns></returns>
        public async Task HandleAsync(IContext context, Either<Exception, UpdateTenantApplicationRequestCommand> command)
        {
            await command.MatchAsync(async cmd =>
            {
                ...
                {
                    Expression<Func<TenantApplicationRequest, bool>> expression = tar => tar.TenantId == cmd.TenantId;
                ...
                {
                    applicationRequest = await _tenantApplicationRequestRepository.GetByIdAsync(cmd.ApplicationId);
                    if (applicationRequest == null)
                    {
                        _logger.LogError("Application Request {ApplicationId} dont exist", cmd.ApplicationId);
                        return;
                    }
                }
                ...
                if (tenant == null)
                {
                    _logger.LogError("Tenant {TenantId} dont exist", cmd.TenantId);
                    return;
                }

                var tenantFullName = $"{tenant.FirstName} {tenant.LastName}";
                applicationDetails = await ValidateApplicationDetails(cmd, applicationDetails);

                if (applicationDetails == null)
                {
                    _logger.LogError("Currently dont exist and application detail for the tenant {TenantId}", cmd.TenantId);
                    return;
                }
                ...
                if (cmd.Coapplicants != null && cmd.Coapplicants.Any())
                {
                    Dictionary<string, Guid?> emailsToSend = new Dictionary<string, Guid?>();
                    cmd.Coapplicants.ForEach(ca =>
                    {
                ...
                {
                    await AddNewGuarantor(cmd, applicationRequest, tenantFullName, subject);
                }
            ...
            {
                var objectEvent = new TenantApplicationRequestUpdatedEvent(Guid.Empty);
                var notificationContext = _contextFactory.Create(string.Empty, context.CreatedBy);
                var messageOutboxId = await _messageService.SendAsync(notificationContext, objectEvent);
                throw ex;
            });
        ...
        private async Task AddNewGuarantor(UpdateTenantApplicationRequestCommand command, TenantApplicationRequest applicationRequest, string tenantFullName, string subject)
        {
            ...
            if (command.Guarantor != null)
            {
                ...
                {
                    Id = null,
                    FirstName = command.Guarantor.FirstName,
                    Email = command.Guarantor.Email,
                    InvitedOn = DateTimeOffset.Now,
                    Status = (int)TenantCoapplicantStatus.Invited
                };
        ...
        private async Task SendEmailInvitation(string email, string cosignerName, string tenantFullName, string subject, Guid? coapplicantId, Guid applicationRequestId, string role)
        {
            ...
            await _otpHandler.SetOtpProcessAsync(otpId, applicationRequestId, Guid.Parse(links[1]), email, email, "Invite",true,false);
