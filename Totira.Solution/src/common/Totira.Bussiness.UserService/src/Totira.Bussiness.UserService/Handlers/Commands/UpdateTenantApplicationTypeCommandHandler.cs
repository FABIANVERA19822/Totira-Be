...
namespace Totira.Bussiness.UserService.Handlers.Commands
{
    ...
    public class UpdateTenantApplicationTypeCommandHandler : IMessageHandler<UpdateTenantApplicationTypeCommand>
    {
        ...
        public async Task HandleAsync(IContext context, UpdateTenantApplicationTypeCommand command)
        {
            ...
            Expression<Func<TenantApplicationRequest, bool>> expressionApplication = (r => r.TenantId == command.TenantId);
            ...
            if ( applicationRequest is not null && command.ApplicationType.Equals(ApplicationTypesEnum.Single.GetEnumDescription()))
            {
                _logger.LogInformation($"Tenant have an application request, it is necessary to eliminate cosigners and emails invitation");

                await ExpiredCoapplicantsSendEmails(applicationRequest, command);
