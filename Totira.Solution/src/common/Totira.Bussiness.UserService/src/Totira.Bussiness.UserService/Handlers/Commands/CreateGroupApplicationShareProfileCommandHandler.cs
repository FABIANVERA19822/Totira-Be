namespace Totira.Bussiness.UserService.Handlers.Commands
{
    ...
    public class CreateGroupApplicationShareProfileCommandHandler : IMessageHandler<CreateGroupApplicationShareProfileCommand>
    {
        ...
        public CreateGroupApplicationShareProfileCommandHandler(ILogger<CreateGroupApplicationShareProfileCommandHandler> logger,
            ...
            IEmailHandler emailHandler, IEncryptionHandler encryptionHandler,
        ...
        public async Task HandleAsync(IContext context, CreateGroupApplicationShareProfileCommand command)
        {
            _logger.LogDebug($"creating tenant GroupApplication Share Profile for application request: {command.ApplicationId}");

            var request = await _applicationRequestRepository.GetByIdAsync(command.ApplicationId);

            if (request == null)
            {
                _logger.LogError($"the ApplicationId {command.ApplicationId} not exist in the DB");
                return;
            }
            ...
            DeleteAllLastShareProfile(command.ApplicationId, command.Email);
            ...
            var tenantGroupApplicationShareProfile = new TenantGroupApplicationShareProfile()
            {
                ...
                Guarantor = request.Guarantor != null && request.Guarantor.Id != null ? new CoapplicantShareProfile()
                {
                    InvinteeType = 2,
            ...
            await _otpHandler.SetOtpProcessAsync(otpId, request.TenantId, null, command.Email, encryptedAccessCode, 1440, "MultiTenant", false, true);

            SendEmail(command, accessCode.ToString(), groupapplicationprofile, request.TenantId, otpId);
            ...
            var messageOutboxId = await _messageService.SendAsync(notificationContext, tenantGroupApplicationShareProfileCreatedEvent);
        }
