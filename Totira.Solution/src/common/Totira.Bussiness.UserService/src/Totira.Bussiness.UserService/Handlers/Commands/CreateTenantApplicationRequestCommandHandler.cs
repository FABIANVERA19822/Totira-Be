...
namespace Totira.Bussiness.UserService.Handlers.Commands
{
    public class CreateTenantApplicationRequestCommandHandler : IMessageHandler<CreateTenantApplicationRequestCommand>
    {
        ...
        public async Task HandleAsync(IContext context, CreateTenantApplicationRequestCommand command)
        {
            ...
            if (command.ToLatest.HasValue && !command.ToLatest.Value)
                prev = (await _tenantApplicationDetailsRepository.GetByIdAsync(command.ApplicationDetailsId.Value)).Id;
            ...
            var tenantApplicationRequestCreatedEvent = new TenantApplicationRequestCreatedEvent(tenantApplicationRequest.Id);
            var notificationContext = _contextFactory.Create(string.Empty, context.CreatedBy);
            var messageOutboxId = await _messageService.SendAsync(notificationContext, tenantApplicationRequestCreatedEvent);
        }
