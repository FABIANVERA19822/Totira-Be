...
namespace Totira.Bussiness.UserService.Handlers.Commands
{
    public class UpdateTenantRentalHistoriesReactivateCommandHandler : IMessageHandler<UpdateTenantRentalHistoriesReactivateCommand>
    {
            ...
            IOtpHandler otpHandler)
        {
            ...
            _tenantBasicInformationRepository = tenantPersonalInformationRepository;
            _emailHandler = emailHandler;
            _settings = settings;
            _otpHandler = otpHandler;
        }

        public async Task HandleAsync(IContext context, Either<Exception, UpdateTenantRentalHistoriesReactivateCommand> command)
        {
            await command.MatchAsync(async cmd =>
            {
                _logger.LogDebug("Update tenant rental history reactivating  for landlord with Id {LandlordId}", cmd.LandlordId);

                Expression<Func<TenantRentalHistories, bool>> expression = p => p.RentalHistories.Where(q => q.Id == cmd.LandlordId).Any();

                var actualData = await _tenantRentalHistoriesRepository.Get(expression);

                var rentalHistoriesInfo = actualData.FirstOrDefault();
                var baseRentalHistories = rentalHistoriesInfo.RentalHistories.Where(p => p.Id == cmd.LandlordId).FirstOrDefault();
                ...
                if (baseRentalHistories != null)
                {
                    ...
                    var tenant = await _tenantBasicInformationRepository.GetByIdAsync(baseRentalHistories.TenantId);
                    ...
                    await _otpHandler.SetOtpProcessAsync(otpId, landLordId, baseRentalHistories.TenantId, baseRentalHistories.ContactInformation.EmailAddress, baseRentalHistories.ContactInformation.EmailAddress, "Feedback", true, false);
