...
{
    public class UpdateTenantEmployeeIncomesCommandValidator : IMessageValidator<UpdateTenantEmployeeIncomesCommand>
    {
        public ValidationResult Validate(UpdateTenantEmployeeIncomesCommand command)
        {
            ...
            //Files
            if (command.NewFiles.Any(x => !allowedTypes.Contains(x.ContentType)))
                errors.Add("Invalid format file. Please upload a supported format file.");
            if (command.NewFiles.Any(x => x.Size > maxFileSize))
