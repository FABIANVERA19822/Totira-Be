...
namespace Totira.Services.RootService.Controllers
{
    ...
    public class VerificationController : DefaultBaseController
    {
        ...
        [HttpPost]
        [Route("applications-property-claims/update/jira")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        public async Task<IActionResult> UpdatePropertyClaimsOnJiraTicket([FromBody] object obj)
        {
            var json = obj.ToString();

            var body = JsonConvert.DeserializeObject<UpdateLandlordPropertyClaimJiraTicketCommand>(json);

            var urlContext = Url.Action("Get", "UpdateTenantEmployeeAndIncomeTicketJiraCommand", new { id = body.Issue.id, version = API_VERSION }) ?? string.Empty;
            var userId = _httpContextAccessor.HttpContext.User?.FindFirst(ClaimTypes.NameIdentifier) != null && Guid.TryParse(_httpContextAccessor.HttpContext.User?.FindFirst(ClaimTypes.NameIdentifier).Value, out Guid userOut) ? userOut : Guid.Empty;
            var context = _contextFactory.Create(urlContext, userId);
