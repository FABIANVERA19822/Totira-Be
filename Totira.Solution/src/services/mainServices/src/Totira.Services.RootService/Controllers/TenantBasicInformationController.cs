namespace Totira.Services.RootService.Controllers
{
    ...
    public class TenantPersonalInfoController : DefaultBaseController
    {
        ...
        {
            var userId = _httpContextAccessor.HttpContext.User?.FindFirst(ClaimTypes.NameIdentifier) != null ? Guid.Parse(_httpContextAccessor.HttpContext.User?.FindFirst(ClaimTypes.NameIdentifier).Value) : info.TenantId;
