    ...
    public class TenantPersonalInfoController : DefaultBaseController
    {
        ...
        public async Task<IActionResult> Post([FromBody] UploadTenantProfileImageDto info)
        {
            ...
            try
            {
                ...
                IFormFile file = AllowedFileExtensionsAttribute.BuildImageFromBase64(info.File, info.ImageName);
            ...
            {
                _logger.LogError($"UploadImageProfile process with userId: {info.TenantId} failed, reason: {ex.Message}");
        ...
        public async Task<IActionResult> UpdateTenantProfileImage([FromForm] UpdateTenantProfileImageDto dto)
        {
            ...
            try
            {
                ...
                if (dto.File != null)
                {
                    AllowedFileExtensionsAttribute.validateImageFormat(stream);
            ...
            {
                _logger.LogError($"UpdateTenantProfileImage process with userId: {dto.TenantId} failed, reason: {ex.Message}");
                return BadRequest(ex.Message);
            }
            return Accepted(context);
        }
