...
namespace Totira.Services.RootService.Events.UserService
{
    ...
    public class TenantStudentFinancialDetailCreatedEvent : IEvent, INotification
    {
        ...
        {
            var info = new TenantStudentFinancialDetailCreatedInfo(this.Id,this.message);
            var json = System.Text.Json.JsonSerializer.Serialize(info);
            return new NotificationMessage(NotificationMessageStatus.Success, json);
        }

        private class TenantStudentFinancialDetailCreatedInfo
        {
            private Guid id;
            public string message { get; set; }

            public TenantStudentFinancialDetailCreatedInfo(Guid id, string message)
            {
                this.id = id;
                this.message = message;
            }
