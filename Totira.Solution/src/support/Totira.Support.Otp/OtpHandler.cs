...
using static Totira.Support.Persistance.IRepository;
    ...
    public class Otphandler : IOtpHandler
    {
        ...
        public async Task SetOtpProcessAsync(
            ...
            bool oneOtp = false)
        {
            await SetOtpProcessAsync(otpId, entityId, null, email, accessCode, expiration, scope, encryptAccessCode, oneOtp);
        }
        ...
        public async Task SetOtpProcessAsync(
            ...
            bool oneOtp = false)
        {
            var expirationInfo = GetOtpOptions(scope);
            await SetOtpProcessAsync(otpId, entityId, entityKey, email, accessCode, expirationInfo.Expiration, scope, encryptAccessCode, oneOtp);
        }
        /// <summary>
        /// Method that is responsible for registering an OTP in the database
        /// </summary>
        ...
        {
            var expirationInfo = GetOtpOptions(scope);
            await SetOtpProcessAsync(otpId, entityId, null, email, accessCode, expirationInfo.Expiration, scope, encryptAccessCode, oneOtp);
        }
        ...
        private  OtpOptions.ScopeItem GetOtpOptions(string scope)
        {
            return _otpOptions.Scopes.FirstOrDefault(x => x.Id == scope)?? new OtpOptions.ScopeItem { Id = "default", Expiration = 1440 }  ;
        }
    }
}