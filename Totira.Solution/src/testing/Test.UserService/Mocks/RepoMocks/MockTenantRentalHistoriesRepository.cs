...
namespace Test.UserService.Mocks.RepoMocks
{
    public static class MockTenantRentalHistoriesRepository
    {
        public static Mock<IRepository<TenantRentalHistories, Guid>> GetTenantRentalHistoriesRepository()
        {
                ...
                new TenantRentalHistories
                {
                    ...
                    RentalHistories = new List<TenantRentalHistory>
                    {
                        new TenantRentalHistory
                        {
                            ...
                            Unit = "House",
                            ContactInformation = new RentalHistoryLandlordContactInformation("test", "LandlorTest", "More Test",
            ...
            mockRepo.Setup(r => r.Get(It.IsAny<Expression<Func<TenantRentalHistories, bool>>>()))
                    .ReturnsAsync((Expression<Func<TenantRentalHistories, bool>> x) => tentantRentalHistorial.Where(x.Compile()));
        ...
        public static Mock<IRepository<TenantRentalHistories, Guid>> GetTenantRentalSingleHistoryRepository()
        {
            ...
            mockRepo.Setup(r => r.Get(It.IsAny<Expression<Func<TenantRentalHistories, bool>>>()))
                    .ReturnsAsync((Expression<Func<TenantRentalHistories, bool>> x) => baseMock.Where(x.Compile()));

            mockRepo.Setup(r => r.GetByIdAsync(It.IsAny<Guid>()))
                    .ReturnsAsync((Guid id) => baseMock.Where(x => x.Id == id).SingleOrDefault());

            mockRepo.Setup(r => r.Add(It.IsAny<TenantRentalHistories>()))
                    .Callback<TenantRentalHistories>(sa => baseMock.Add(sa));

            mockRepo.Setup(r => r.Update(It.IsAny<TenantRentalHistories>()))
                    .Callback<TenantRentalHistories>(sa => baseMock.Where(x => x.Id == sa.Id));
